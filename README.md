...
2. For both cancer drugs approved for reimbursement and national sact regimens, files with extensions `_edited_or_removed_rows.json` and `_new_rows.json` will be produced. With these files, update `hse-cache.json` manually.
3. Annotate new entries in [hse-cache.json](hse-cache.json) with "1" for the biomarker key, if the entry contains at least 1 biomarker, and set the "cataloged" key to be 1 after it has been added to [moalmanac-db](https://github.com/vanallenlab/moalmanac-db). 
